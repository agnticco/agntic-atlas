...
**This is the honest record of how Atlas was built and, mostly, what went wrong
while building it.** Decisions that are closed and should not be reopened, code
that should not be refactored without a reason, and — the bulk of it — a
chronological account of every significant defect that reached users: what it
looked like from their side, what actually caused it, what it cost, and the rule
that came out of it.

**Why publish this at all.** Most projects' history lives in commit messages that
say "fix: idempotency bug". That is useless six months later. Almost everything
below was expensive to learn, and a striking amount of it is the *same few
mistakes* wearing different clothes — one rule written in two places that drifted
apart, a check scoped to the shape of a value rather than what it means, a failure
that looked like success. If you are working on this codebase, the fastest way to
avoid repeating them is to read them.

**If something here is wrong, fix this file in the same commit as the code.** A
document that disagrees with `main` is worse than no document: it reads as
authoritative, and the next person builds on it.
...
- **Whoever writes the code usually writes its tests, so the two share blind spots.**
  A test written from the same mistaken picture of the system confirms the picture
  rather than the behaviour. Independent review — by someone who did not write the
  code and is willing to reject it — is the remaining defence. Do not treat your own
  green suite as the last word.
...
- **Re-ground every remediation brief against live code at the moment you execute it.** Line
  ...
  a behavioural test is the contract. Believe fresh grounding against live code over a
...
app; the team triages in the admin app and hands off for a fix (Markdown brief
...
## Usage metering — OFF by default, and that is the important part

**Atlas runs SELF-HOSTED unless told otherwise, and self-hosted means no limits at
all** — unlimited workflows, runs and seats, and no daily spend ceiling
(`isSelfHosted` / `effectivePlan` in `src/entitlements/index.js`, default `true`).
Someone running Atlas on their own machine pays for every token directly, so
metering them is meaningless, and the stored default plan would otherwise cap them
at one workflow — a crippled product out of the box. `ATLAS_SELF_HOSTED=false` opts
a deployment IN to metering; the server prints which mode it booted in on every
start, so an uncapped public deployment can never be a silent surprise.

**The metering machinery below is dormant unless that lever is thrown.** It exists
so Atlas *can* be run as a service by anyone who wants to. Tiering is
**volume-only — no feature-matrix gating** (every feature on every plan). The
**loud constraint is `activeWorkflows`**: the publish gate
(`POST /api/builder/workflows`) blocks a live workflow over the cap with a 402
`PLAN_LIMIT`, which the UI turns into an immediate Upgrade modal. `monthlyRuns` is
a **hard** per-tenant/month cap enforced at the single
`WorkflowScheduler._executeFlow` choke point (`registerRunBudgetCheck`, fail-open);
**test runs are exempt** (they never count or block). Upgrade is **real Stripe
Checkout** (`src/billing/stripe.js`, routes `/api/billing/checkout|portal` +
`/webhooks/stripe`), env-driven and **fails soft with no keys** (app boots;
checkout 503s). The actual numbers live in `PLANS` — never copy them into a doc,
which is how the tier-gating doc came to claim three times the real workflow
allowance.
...
### Self-serve signup is OFF by default — a workspace is created for you

**The default:** nobody provisions themselves. Workspaces are created by hand
(`POST /admin/tenants`, platform-admin gated) and plans are **granted**, not bought
(`tenantStore.setPlan(id, 'business')` — unlimited everywhere, and deliberately excluded from
`SELF_SERVE_PLANS` because an unlimited plan a card can buy is an unbounded cost liability).
For a self-hosted install this is simply how it works: you complete the first-run setup, you
are the platform admin, and you create workspaces for whoever needs one.
...
- **`server.js` encoding.** In `agntic-prod`, `src/api/server.js` reads as
  ...
  a reader the engine was deleted. **Root cause (verified):** the file is valid
...
- **A build OFFER with no Build it button was SPECIFIED, not broken — and the envelope retry
  ...
  **The retry is healthy and has NEVER FIRED.** `envelopeRetried` in `src/api/builder.js` (landed
  ...
  every turn. **Nothing was recovering, because nothing was broken to recover.** ENGINEERING-LOG.md
...
**This section describes ONE reference deployment.** Nothing here is required — Atlas
runs from `npm start` on a laptop or from `docker compose up`. It is kept because the
shape has been exercised in real use and is a reasonable pattern to copy: substitute
your own host for `YOUR_SERVER_IP` throughout.

That deployment is a single Node process (`systemd` unit `atlas`) behind a Cloudflare
Tunnel (`cloudflared`) on a small Ubuntu box. The app lives at `/home/atlas/atlas`,
owned by an unprivileged `atlas` service account. You **SSH in as `ubuntu`** (there
is no `atlas` SSH login) and **`sudo -u atlas`** to act as the run user. Secrets live
in `/home/atlas/atlas/.env` **on the box** (gitignored, never committed); the
operator manages them, and they are never entered or echoed anywhere else.
...
- **EVERY live UI verification is done in a VISIBLE browser the operator can watch
  ...
  frame is saved rather than merely described. The point
  ...
  it being claimed. If a headed window cannot be opened, say so and stop, rather
...
- **The doc is the memory — keep it true, in the same commit.** Fresh sessions only
  work because the *documents* carry the state, not anyone's memory. So: if your work
...
- **Evidence-gating (load-bearing).** Never report something missing,
...
- **Independent review at every gate.** A gate is closed by someone who did *not*
...
*(Plain language first; the technical anchors a reader
...
**not** closed by a passing checkpoint and **not** independently reviewed, and the
...
Since 2026-07-22 there has been a dedicated QA practice, separate from writing the
code, whose whole job is to *use* the live product like a customer — in a visible
browser, on the real deployment — and report what misbehaves. It never edits code.

The reason it exists is the single most useful fact in this document: **every serious
defect this codebase has shipped was found by USING the product, not by reading it,
with the test suite green throughout.** Reviewing the diff would not have found them;
someone clicking the button did. Running record:
...
**Fixed by the coding run of 2026-07-26** (a five-packet fix run,
...
1. **The user's answer does not take effect — RE-OPENED 2026-07-26.** *(Previously
   ...
   *"Closed with 3 — re-measured: 0 re-asks, 1 build pass"*, which measured the FIRST half
   ...
   - **Half NOT fixed, and it is the half that matters:** the answer still does not change
     ...
     the QA records for that run, finding 6.
...
- [x] **P12 — the promise system.** *Closed 2026-07-20 by operator decision —
  ...
  **How it closed, stated honestly.** The operator closed it because the work was
  ...
  The checkpoint script was not run to completion and nobody independently reviewed
...
- [~] **P13 — many more connectors.** *IN PROGRESS. P13-0 merged 2026-07-25; **P13-A
  ...
  **3 — THE REFUSAL WAS A BLOCKING BROWSER POPUP WITH NO REASON.** Go live failed via
  ...
  a browser modal FREEZES THE WHOLE PAGE until dismissed (it froze the test tooling
  ...
  **Two things review caught that the author's own green suite did not**, both
  ...
  **No coverage percentage blocks this phase.** Re-breaking the code to check a
  ...
  - *Design: [`docs/architecture/mcp-capability-adapter.md`](docs/architecture/mcp-capability-adapter.md);
    ...
    `Phase:` + `Verified-by:` from an independent reviewer who did not write the code.*
