...
export class BaseMessage {
  ...
  static fromJSON(json) {
    ...
    let message;
    if (json.type === 'tool') {
      message = new MessageClass(json.content, json.toolCallId, json.additionalKwargs ?? {});
    } else if (json.type === 'ai') {
      message = new MessageClass(json.content, {
        ...(json.additionalKwargs ?? {}),
        toolCalls: json.toolCalls ?? [],
      });
    } else {
      message = new MessageClass(json.content, json.additionalKwargs ?? {});
    }

    if (json.id)        message.id        = json.id;
    if (json.timestamp) message.timestamp = json.timestamp;
    return message;
  }
...
export function mergeConsecutiveMessages(messages) {
  ...
  for (let i = 1; i < messages.length; i++) {
    ...
    if (
      ...
      const MessageClass = MESSAGE_TYPES[current._type];
