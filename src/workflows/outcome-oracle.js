...
// ── THE RUN-SCORING LAYER WAS REMOVED HERE (2026-08-02, Charles's call) ──────
//
// Four functions lived below this line and are gone:
//
//   evaluateExampleRun      — scored one run against `outcome.assertions`
//   assertionApplicability  — which promises a run's lane made applicable
//   approvalAskEvidence     — an unissued approval question is not a broken promise
//   emptyLoopEvidence       — a foreach over an empty collection proved nothing
//
// They implemented "did this run keep its promise", by comparing each delivery's
// connector and locator STRING against the promise's `target`. The test now asks
// a different question — did every step complete, and did every delivery land —
// and that question is answered in `src/workflows/delivery-verdict.js`. Read its
// header for the measurements behind the decision.
//
// WHAT DID NOT GO, AND WHY IT IS STILL HERE. `satisfiesAssertion` and
// `checkAssertionAtRuntime` remain, because the BUILD-time checks still use them:
// the validator's `checkOutcome`, the gap scorer, escalation, and the
// self-consistency reporter. Removing the run-scoring layer does not touch what
// the interview does with a promise before the workflow exists — that is a
// separate decision, and it has not been made.
//
// Also gone with them: the third verdict. A run is now 'kept' or 'broken'. The
// three shapes 'not_exercised' existed for — a spec with no assertions, a sample
// that took a do-nothing lane, a "should not fire" example — are all simply runs
// that completed, and a run that completes and breaks nothing is not a failure.
// The one thing it could say that nothing else can — "this whole set proved
// nothing" — moved to the callers, as "no delivery was attempted anywhere".
