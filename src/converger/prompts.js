...
- llm: THE AI STEP. One node type does every AI job; \`mode\` picks which. Emit exactly one mode per node:
  ...
  IMPORTANT: when an llm node consumes connector-action output (Drive files, Airtable records, etc.), its instructions/prompt MUST begin with a guard clause, e.g.: "If the provided data is ENTIRELY absent — an empty list, no records, no message at all — output EXACTLY: ERROR: required data not found — do not compose content. Any content, however short, counts as present: work with what you are given." This prevents silent hallucination when a connector returns no results, WITHOUT refusing a real but brief input.

  THE SECOND HALF OF THAT SENTENCE IS LOAD-BEARING. Witnessed on prod 2026-08-03: a Slack-triggered workflow was handed a real message whose body was the word "test", the model judged there was nothing worth summarising, emitted the sentinel, and the run was recorded as a FAILURE. The plumbing was perfect — the context arrived intact, 62 characters, correctly formatted. A person testing their own automation by posting "test" is the commonest thing anyone does, and the product told them it was broken. The guard exists to stop invention when there is NOTHING; it must never fire on something short.
