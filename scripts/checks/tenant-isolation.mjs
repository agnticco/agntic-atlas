...
try {
  ...
  const setup = await api('POST', '/setup', { body: { token: spine.auth.bootstrap.token, email: 'ops@atlas.dev', password: 'platform-pw-1' } });
  ...
  const c1 = await api('POST', '/tenants', { token: platformToken, body: { name: 'Acme', slug: 'acme', admin: { email: 'a@acme.test', password: 'acme-pw-12' } } });
  const c2 = await api('POST', '/tenants', { token: platformToken, body: { name: 'Globex', slug: 'globex', admin: { email: 'b@globex.test', password: 'globex-pw-1' } } });
  ok('platform created two tenants', c1.status === 200 && c2.status === 200);

  // Tenant admins log in.
  const la = await api('POST', '/auth/login', { body: { email: 'a@acme.test', password: 'acme-pw-12' } });
  const lb = await api('POST', '/auth/login', { body: { email: 'b@globex.test', password: 'globex-pw-1' } });
  ...
  const dup = await api('POST', '/tenants/acme/users', { token: platformToken, body: { email: 'b@globex.test', password: 'x-pw-1234' } });
  ...
  const reLogin = await api('POST', '/auth/login', { body: { email: 'a@acme.test', password: 'acme-pw-12' } });
